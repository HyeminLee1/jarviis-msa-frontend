...
export default function TaskAdd() {
    ...
    return (
        <div className="todoAdd stack-large">
        {mode == 0 ? <>
            <button type="button" className="btn" variant="outlined" onClick={() => setMode(1)}> ADD</button>
        </>
            : <Box
                component="form"
                sx={{
                ...
                noValidate
                autoComplete="off"
                onSubmit= {handleSubmit(async (data) => { await dispatch(taskRequest({
                    ...data,
                   })) })}
                onSubmit={handleSubmit(async (data) => console.log(JSON.stringify(data)))}
            >
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            ...
                            >
                                ...
                                <MenuItem value={'payment'}>기타</MenuItem>
                            </Select>
                        )} />
                </FormControl>
                <Controller
